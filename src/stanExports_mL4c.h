...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "model_mL4c");
    reader.add_event(240, 238, "end", "model_mL4c");
    return reader;
}
...
class model_mL4c
  : public stan::model::model_base_crtp<model_mL4c> {
private:
        int N;
        std::vector<int> n;
        int nc;
            ...
            if (as_bool((primitive_value(logical_eq(data_type, 1)) || primitive_value(logical_eq(data_type, 3))))) {
                ...
                for (int i = 1; i <= N; ++i) {
                    ...
                    if (as_bool(logical_eq(data_type, 1))) {
                        ...
                        stan::math::assign(mx, (a * inv_logit(((stan::math::pi() / pow(3, 0.5)) * (c + (b * pow(get_base1(x, i, "x", 1), d)))))));
                    } else if (as_bool(logical_eq(data_type, 3))) {
                        current_statement_begin__ = 141;
                        stan::math::assign(mx, ((a * (1 + inv_logit(((stan::math::pi() / pow(3, 0.5)) * c)))) - (a * inv_logit(((stan::math::pi() / pow(3, 0.5)) * (c + (b * pow(get_base1(x, i, "x", 1), d))))))));
                    }
...
typedef model_mL4c_namespace::model_mL4c stan_model;
#ifndef USING_R
