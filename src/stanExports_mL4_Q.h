...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "model_mL4_Q");
    reader.add_event(103, 101, "end", "model_mL4_Q");
    return reader;
}
...
class model_mL4_Q
  : public stan::model::model_base_crtp<model_mL4_Q> {
private:
        int N;
        vector_d n;
        vector_d x;
...
public:
    ...
    T__ log_prob(std::vector<T__>& params_r__,
        ...
        try {
            ...
            stan::math::assign(b, (stan::math::exp((-(k) * d)) * (((pow(3, 0.5) / stan::math::pi()) * logit(((q * (1 - a)) + a))) - ((pow(3, 0.5) / stan::math::pi()) * logit(a)))));
            ...
            for (int i = 1; i <= N; ++i) {
                ...
                if (as_bool(logical_eq(get_base1(x, i, "x", 1), 0))) {
                    ...
                    stan::model::assign(m, 
                                stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                                inv_logit(((stan::math::pi() / pow(3, 0.5)) * (((pow(3, 0.5) / stan::math::pi()) * logit(a)) + (b * pow(get_base1(x, i, "x", 1), d))))), 
                                "assigning variable m");
                }
            }
...
typedef model_mL4_Q_namespace::model_mL4_Q stan_model;
#ifndef USING_R
