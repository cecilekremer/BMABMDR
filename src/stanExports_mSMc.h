...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "model_mSMc");
    reader.add_event(182, 180, "end", "model_mSMc");
    return reader;
}
...
class model_mSMc
  : public stan::model::model_base_crtp<model_mSMc> {
private:
        int N;
        std::vector<int> n;
        int nc;
...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        try {
            ...
            context__.validate_dims("data initialization", "priormu", "vector_d", context__.to_vec((N + 2)));
            priormu = Eigen::Matrix<double, Eigen::Dynamic, 1>((N + 2));
            vals_r__ = context__.vals_r("priormu");
            pos__ = 0;
            size_t priormu_j_1_max__ = (N + 2);
            ...
            validate_non_negative_index("par", "(N + 2)", (N + 2));
            num_params_r__ += (N + 2);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        pos__ = 0U;
        validate_non_negative_index("par", "(N + 2)", (N + 2));
        context__.validate_dims("parameter initialization", "par", "vector_d", context__.to_vec((N + 2)));
        Eigen::Matrix<double, Eigen::Dynamic, 1> par((N + 2));
        size_t par_j_1_max__ = (N + 2);
    ...
    T__ log_prob(std::vector<T__>& params_r__,
        ...
        try {
            ...
            if (jacobian__)
                par = in__.vector_constrain((N + 2), lp__);
            else
                par = in__.vector_constrain((N + 2));
            ...
            stan::math::assign(rho_cluster, get_base1(par, (N + 2), "par", 1));
            ...
            lp_accum__.add(pert_dist_lpdf<propto__>(get_base1(par, 1, "par", 1), priorlb, get_base1(priormu, 1, "priormu", 1), get_base1(priorub, 1, "priorub", 1), priorg, pstream__));
            ...
            lp_accum__.add(normal_log<propto__>(get_base1(par, (N + 1), "par", 1), get_base1(priormu, (N + 1), "priormu", 1), get_base1(priorSigma, (N + 1), (N + 1), "priorSigma", 1)));
            current_statement_begin__ = 75;
            lp_accum__.add(pert_dist_lpdf<propto__>(get_base1(par, (N + 2), "par", 1), 0, 0.5, 1, 0.0001, pstream__));
            ...
            if (as_bool((primitive_value(logical_eq(data_type, 1)) || primitive_value(logical_eq(data_type, 3))))) {
                ...
                for (int i = 1; i <= N; ++i) {
                    ...
                    for (int j = 1; j <= nl; ++j) {
                        ...
                        stan::math::assign(m, rep_row_vector(get_base1(a, i, "a", 1), lt));
    ...
    void write_array(RNG& base_rng__,
        ...
        Eigen::Matrix<double, Eigen::Dynamic, 1> par = in__.vector_constrain((N + 2));
        size_t par_j_1_max__ = (N + 2);
        for (size_t j_1__ = 0; j_1__ < par_j_1_max__; ++j_1__) {
...
typedef model_mSMc_namespace::model_mSMc stan_model;
#ifndef USING_R
