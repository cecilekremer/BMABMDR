// Generated by rstantools.  Do not edit by hand.

/*
    BMABMDR is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    BMABMDR is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with BMABMDR.  If not, see <http://www.gnu.org/licenses/>.
*/
#ifndef MODELS_HPP
#define MODELS_HPP
#define STAN__SERVICES__COMMAND_HPP
#include <rstan/rstaninc.hpp>
// Code generated by Stan version 2.21.0
#include <stan/model/model_header.hpp>
namespace model_mG4_Q_COV_namespace {
using std::istream;
using std::string;
using std::stringstream;
using std::vector;
using stan::io::dump;
using stan::math::lgamma;
using stan::model::prob_grad;
using namespace stan::math;
static int current_statement_begin__;
stan::io::program_reader prog_reader__() {
    stan::io::program_reader reader;
    reader.add_event(0, 0, "start", "model_mG4_Q_COV");
    reader.add_event(188, 186, "end", "model_mG4_Q_COV");
    return reader;
}
template <typename T0__, typename T1__, typename T2__>
Eigen::Matrix<typename boost::math::tools::promote_args<T0__, T1__, T2__>::type, Eigen::Dynamic, 1>
algebra_system(const Eigen::Matrix<T0__, Eigen::Dynamic, 1>& yG,
                   const Eigen::Matrix<T1__, Eigen::Dynamic, 1>& theta,
                   const std::vector<T2__>& x_r,
                   const std::vector<int>& x_i, std::ostream* pstream__) {
    typedef typename boost::math::tools::promote_args<T0__, T1__, T2__>::type local_scalar_t__;
    typedef local_scalar_t__ fun_return_scalar_t__;
    const static bool propto__ = true;
    (void) propto__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
    int current_statement_begin__ = -1;
    try {
        {
        current_statement_begin__ = 6;
        validate_non_negative_index("x", "1", 1);
        Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> x(1);
        stan::math::initialize(x, DUMMY_VAR__);
        stan::math::fill(x, DUMMY_VAR__);
        current_statement_begin__ = 7;
        local_scalar_t__ q(DUMMY_VAR__);
        (void) q;  // dummy to suppress unused var warning
        stan::math::initialize(q, DUMMY_VAR__);
        stan::math::fill(q, DUMMY_VAR__);
        stan::math::assign(q,get_base1(x_r, 1, "x_r", 1));
        current_statement_begin__ = 8;
        if (as_bool(logical_gt(get_base1(yG, 1, "yG", 1), 0))) {
            current_statement_begin__ = 8;
            stan::model::assign(x, 
                        stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                        (gamma_p(get_base1(theta, 2, "theta", 1), (get_base1(yG, 1, "yG", 1) * get_base1(theta, 1, "theta", 1))) - q), 
                        "assigning variable x");
        } else if (as_bool(logical_lte(get_base1(yG, 1, "yG", 1), 0))) {
            current_statement_begin__ = 9;
            stan::model::assign(x, 
                        stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                        1, 
                        "assigning variable x");
        }
        current_statement_begin__ = 10;
        return stan::math::promote_scalar<fun_return_scalar_t__>(x);
        }
    } catch (const std::exception& e) {
        stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
        // Next line prevents compiler griping about no return
        throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
    }
}
struct algebra_system_functor__ {
    template <typename T0__, typename T1__, typename T2__>
        Eigen::Matrix<typename boost::math::tools::promote_args<T0__, T1__, T2__>::type, Eigen::Dynamic, 1>
    operator()(const Eigen::Matrix<T0__, Eigen::Dynamic, 1>& yG,
                   const Eigen::Matrix<T1__, Eigen::Dynamic, 1>& theta,
                   const std::vector<T2__>& x_r,
                   const std::vector<int>& x_i, std::ostream* pstream__) const {
        return algebra_system(yG, theta, x_r, x_i, pstream__);
    }
};
template <bool propto, typename T0__, typename T1__, typename T2__, typename T3__, typename T4__>
typename boost::math::tools::promote_args<T0__, T1__, T2__, T3__, typename boost::math::tools::promote_args<T4__>::type>::type
pert_dist_lpdf(const T0__& theta,
                   const T1__& lb,
                   const T2__& md,
                   const T3__& ub,
                   const T4__& gama, std::ostream* pstream__) {
    typedef typename boost::math::tools::promote_args<T0__, T1__, T2__, T3__, typename boost::math::tools::promote_args<T4__>::type>::type local_scalar_t__;
    typedef local_scalar_t__ fun_return_scalar_t__;
    const static bool propto__ = true;
    (void) propto__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
    int current_statement_begin__ = -1;
    try {
        {
        current_statement_begin__ = 13;
        local_scalar_t__ x1(DUMMY_VAR__);
        (void) x1;  // dummy to suppress unused var warning
        stan::math::initialize(x1, DUMMY_VAR__);
        stan::math::fill(x1, DUMMY_VAR__);
        current_statement_begin__ = 14;
        local_scalar_t__ x2(DUMMY_VAR__);
        (void) x2;  // dummy to suppress unused var warning
        stan::math::initialize(x2, DUMMY_VAR__);
        stan::math::fill(x2, DUMMY_VAR__);
        current_statement_begin__ = 15;
        local_scalar_t__ x3(DUMMY_VAR__);
        (void) x3;  // dummy to suppress unused var warning
        stan::math::initialize(x3, DUMMY_VAR__);
        stan::math::fill(x3, DUMMY_VAR__);
        current_statement_begin__ = 16;
        local_scalar_t__ x4(DUMMY_VAR__);
        (void) x4;  // dummy to suppress unused var warning
        stan::math::initialize(x4, DUMMY_VAR__);
        stan::math::fill(x4, DUMMY_VAR__);
        current_statement_begin__ = 17;
        local_scalar_t__ alpha(DUMMY_VAR__);
        (void) alpha;  // dummy to suppress unused var warning
        stan::math::initialize(alpha, DUMMY_VAR__);
        stan::math::fill(alpha, DUMMY_VAR__);
        current_statement_begin__ = 18;
        local_scalar_t__ beta(DUMMY_VAR__);
        (void) beta;  // dummy to suppress unused var warning
        stan::math::initialize(beta, DUMMY_VAR__);
        stan::math::fill(beta, DUMMY_VAR__);
        current_statement_begin__ = 20;
        stan::math::assign(alpha, (1 + ((gama * (md - lb)) / (ub - lb))));
        current_statement_begin__ = 21;
        stan::math::assign(beta, (1 + ((gama * (ub - md)) / (ub - lb))));
        current_statement_begin__ = 23;
        stan::math::assign(x1, ((alpha - 1) * stan::math::log((theta - lb))));
        current_statement_begin__ = 24;
        stan::math::assign(x2, ((beta - 1) * stan::math::log((ub - theta))));
        current_statement_begin__ = 25;
        stan::math::assign(x3, (((alpha + beta) - 1) * stan::math::log((ub - lb))));
        current_statement_begin__ = 26;
        stan::math::assign(x4, lbeta(alpha, beta));
        current_statement_begin__ = 27;
        return stan::math::promote_scalar<fun_return_scalar_t__>((((x1 + x2) - x3) - x4));
        }
    } catch (const std::exception& e) {
        stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
        // Next line prevents compiler griping about no return
        throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
    }
}
template <typename T0__, typename T1__, typename T2__, typename T3__, typename T4__>
typename boost::math::tools::promote_args<T0__, T1__, T2__, T3__, typename boost::math::tools::promote_args<T4__>::type>::type
pert_dist_lpdf(const T0__& theta,
                   const T1__& lb,
                   const T2__& md,
                   const T3__& ub,
                   const T4__& gama, std::ostream* pstream__) {
    return pert_dist_lpdf<false>(theta,lb,md,ub,gama, pstream__);
}
struct pert_dist_lpdf_functor__ {
    template <bool propto, typename T0__, typename T1__, typename T2__, typename T3__, typename T4__>
        typename boost::math::tools::promote_args<T0__, T1__, T2__, T3__, typename boost::math::tools::promote_args<T4__>::type>::type
    operator()(const T0__& theta,
                   const T1__& lb,
                   const T2__& md,
                   const T3__& ub,
                   const T4__& gama, std::ostream* pstream__) const {
        return pert_dist_lpdf(theta, lb, md, ub, gama, pstream__);
    }
};
#include <stan_meta_header.hpp>
class model_mG4_Q_COV
  : public stan::model::model_base_crtp<model_mG4_Q_COV> {
private:
        int N;
        int nlevels;
        int nlevels_a;
        int nlevels_BMD;
        int nlevels_d;
        int nlevels_b;
        vector_d n;
        vector_d x;
        vector_d y;
        matrix_d trt_ind;
        double q;
        double init_b;
        matrix_d priormu;
        matrix_d priorlb;
        matrix_d priorub;
        matrix_d priorgama;
        double eps;
        matrix_d priorSigma;
        double truncd;
        std::vector<double> x_r;
        std::vector<int> x_i;
public:
    model_mG4_Q_COV(stan::io::var_context& context__,
        std::ostream* pstream__ = 0)
        : model_base_crtp(0) {
        ctor_body(context__, 0, pstream__);
    }
    model_mG4_Q_COV(stan::io::var_context& context__,
        unsigned int random_seed__,
        std::ostream* pstream__ = 0)
        : model_base_crtp(0) {
        ctor_body(context__, random_seed__, pstream__);
    }
    void ctor_body(stan::io::var_context& context__,
                   unsigned int random_seed__,
                   std::ostream* pstream__) {
        typedef double local_scalar_t__;
        boost::ecuyer1988 base_rng__ =
          stan::services::util::create_rng(random_seed__, 0);
        (void) base_rng__;  // suppress unused var warning
        current_statement_begin__ = -1;
        static const char* function__ = "model_mG4_Q_COV_namespace::model_mG4_Q_COV";
        (void) function__;  // dummy to suppress unused var warning
        size_t pos__;
        (void) pos__;  // dummy to suppress unused var warning
        std::vector<int> vals_i__;
        std::vector<double> vals_r__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
        try {
            // initialize data block variables from context__
            current_statement_begin__ = 31;
            context__.validate_dims("data initialization", "N", "int", context__.to_vec());
            N = int(0);
            vals_i__ = context__.vals_i("N");
            pos__ = 0;
            N = vals_i__[pos__++];
            current_statement_begin__ = 32;
            context__.validate_dims("data initialization", "nlevels", "int", context__.to_vec());
            nlevels = int(0);
            vals_i__ = context__.vals_i("nlevels");
            pos__ = 0;
            nlevels = vals_i__[pos__++];
            current_statement_begin__ = 33;
            context__.validate_dims("data initialization", "nlevels_a", "int", context__.to_vec());
            nlevels_a = int(0);
            vals_i__ = context__.vals_i("nlevels_a");
            pos__ = 0;
            nlevels_a = vals_i__[pos__++];
            current_statement_begin__ = 34;
            context__.validate_dims("data initialization", "nlevels_BMD", "int", context__.to_vec());
            nlevels_BMD = int(0);
            vals_i__ = context__.vals_i("nlevels_BMD");
            pos__ = 0;
            nlevels_BMD = vals_i__[pos__++];
            current_statement_begin__ = 35;
            context__.validate_dims("data initialization", "nlevels_d", "int", context__.to_vec());
            nlevels_d = int(0);
            vals_i__ = context__.vals_i("nlevels_d");
            pos__ = 0;
            nlevels_d = vals_i__[pos__++];
            current_statement_begin__ = 36;
            context__.validate_dims("data initialization", "nlevels_b", "int", context__.to_vec());
            nlevels_b = int(0);
            vals_i__ = context__.vals_i("nlevels_b");
            pos__ = 0;
            nlevels_b = vals_i__[pos__++];
            current_statement_begin__ = 37;
            validate_non_negative_index("n", "N", N);
            context__.validate_dims("data initialization", "n", "vector_d", context__.to_vec(N));
            n = Eigen::Matrix<double, Eigen::Dynamic, 1>(N);
            vals_r__ = context__.vals_r("n");
            pos__ = 0;
            size_t n_j_1_max__ = N;
            for (size_t j_1__ = 0; j_1__ < n_j_1_max__; ++j_1__) {
                n(j_1__) = vals_r__[pos__++];
            }
            current_statement_begin__ = 38;
            validate_non_negative_index("x", "N", N);
            context__.validate_dims("data initialization", "x", "vector_d", context__.to_vec(N));
            x = Eigen::Matrix<double, Eigen::Dynamic, 1>(N);
            vals_r__ = context__.vals_r("x");
            pos__ = 0;
            size_t x_j_1_max__ = N;
            for (size_t j_1__ = 0; j_1__ < x_j_1_max__; ++j_1__) {
                x(j_1__) = vals_r__[pos__++];
            }
            current_statement_begin__ = 39;
            validate_non_negative_index("y", "N", N);
            context__.validate_dims("data initialization", "y", "vector_d", context__.to_vec(N));
            y = Eigen::Matrix<double, Eigen::Dynamic, 1>(N);
            vals_r__ = context__.vals_r("y");
            pos__ = 0;
            size_t y_j_1_max__ = N;
            for (size_t j_1__ = 0; j_1__ < y_j_1_max__; ++j_1__) {
                y(j_1__) = vals_r__[pos__++];
            }
            current_statement_begin__ = 40;
            validate_non_negative_index("trt_ind", "N", N);
            validate_non_negative_index("trt_ind", "nlevels", nlevels);
            context__.validate_dims("data initialization", "trt_ind", "matrix_d", context__.to_vec(N,nlevels));
            trt_ind = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(N, nlevels);
            vals_r__ = context__.vals_r("trt_ind");
            pos__ = 0;
            size_t trt_ind_j_2_max__ = nlevels;
            size_t trt_ind_j_1_max__ = N;
            for (size_t j_2__ = 0; j_2__ < trt_ind_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < trt_ind_j_1_max__; ++j_1__) {
                    trt_ind(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 41;
            context__.validate_dims("data initialization", "q", "double", context__.to_vec());
            q = double(0);
            vals_r__ = context__.vals_r("q");
            pos__ = 0;
            q = vals_r__[pos__++];
            current_statement_begin__ = 42;
            context__.validate_dims("data initialization", "init_b", "double", context__.to_vec());
            init_b = double(0);
            vals_r__ = context__.vals_r("init_b");
            pos__ = 0;
            init_b = vals_r__[pos__++];
            current_statement_begin__ = 43;
            validate_non_negative_index("priormu", "4", 4);
            validate_non_negative_index("priormu", "nlevels", nlevels);
            context__.validate_dims("data initialization", "priormu", "matrix_d", context__.to_vec(4,nlevels));
            priormu = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(4, nlevels);
            vals_r__ = context__.vals_r("priormu");
            pos__ = 0;
            size_t priormu_j_2_max__ = nlevels;
            size_t priormu_j_1_max__ = 4;
            for (size_t j_2__ = 0; j_2__ < priormu_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < priormu_j_1_max__; ++j_1__) {
                    priormu(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 44;
            validate_non_negative_index("priorlb", "4", 4);
            validate_non_negative_index("priorlb", "nlevels", nlevels);
            context__.validate_dims("data initialization", "priorlb", "matrix_d", context__.to_vec(4,nlevels));
            priorlb = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(4, nlevels);
            vals_r__ = context__.vals_r("priorlb");
            pos__ = 0;
            size_t priorlb_j_2_max__ = nlevels;
            size_t priorlb_j_1_max__ = 4;
            for (size_t j_2__ = 0; j_2__ < priorlb_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < priorlb_j_1_max__; ++j_1__) {
                    priorlb(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 45;
            validate_non_negative_index("priorub", "4", 4);
            validate_non_negative_index("priorub", "nlevels", nlevels);
            context__.validate_dims("data initialization", "priorub", "matrix_d", context__.to_vec(4,nlevels));
            priorub = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(4, nlevels);
            vals_r__ = context__.vals_r("priorub");
            pos__ = 0;
            size_t priorub_j_2_max__ = nlevels;
            size_t priorub_j_1_max__ = 4;
            for (size_t j_2__ = 0; j_2__ < priorub_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < priorub_j_1_max__; ++j_1__) {
                    priorub(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 46;
            validate_non_negative_index("priorgama", "2", 2);
            validate_non_negative_index("priorgama", "nlevels", nlevels);
            context__.validate_dims("data initialization", "priorgama", "matrix_d", context__.to_vec(2,nlevels));
            priorgama = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(2, nlevels);
            vals_r__ = context__.vals_r("priorgama");
            pos__ = 0;
            size_t priorgama_j_2_max__ = nlevels;
            size_t priorgama_j_1_max__ = 2;
            for (size_t j_2__ = 0; j_2__ < priorgama_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < priorgama_j_1_max__; ++j_1__) {
                    priorgama(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 47;
            context__.validate_dims("data initialization", "eps", "double", context__.to_vec());
            eps = double(0);
            vals_r__ = context__.vals_r("eps");
            pos__ = 0;
            eps = vals_r__[pos__++];
            current_statement_begin__ = 48;
            validate_non_negative_index("priorSigma", "3", 3);
            validate_non_negative_index("priorSigma", "3", 3);
            context__.validate_dims("data initialization", "priorSigma", "matrix_d", context__.to_vec(3,3));
            priorSigma = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(3, 3);
            vals_r__ = context__.vals_r("priorSigma");
            pos__ = 0;
            size_t priorSigma_j_2_max__ = 3;
            size_t priorSigma_j_1_max__ = 3;
            for (size_t j_2__ = 0; j_2__ < priorSigma_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < priorSigma_j_1_max__; ++j_1__) {
                    priorSigma(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            stan::math::check_cov_matrix(function__, "priorSigma", priorSigma);
            current_statement_begin__ = 49;
            context__.validate_dims("data initialization", "truncd", "double", context__.to_vec());
            truncd = double(0);
            vals_r__ = context__.vals_r("truncd");
            pos__ = 0;
            truncd = vals_r__[pos__++];
            // initialize transformed data variables
            current_statement_begin__ = 52;
            validate_non_negative_index("x_r", "1", 1);
            x_r = std::vector<double>(1, double(0));
            stan::math::fill(x_r, DUMMY_VAR__);
            stan::math::assign(x_r,static_cast<std::vector<double> >(stan::math::array_builder<double >().add(q).array()));
            current_statement_begin__ = 53;
            validate_non_negative_index("x_i", "0", 0);
            x_i = std::vector<int>(0, int(0));
            stan::math::fill(x_i, std::numeric_limits<int>::min());
            // execute transformed data statements
            // validate transformed data
            // validate, set parameter ranges
            num_params_r__ = 0U;
            param_ranges_i__.clear();
            current_statement_begin__ = 56;
            validate_non_negative_index("par1", "nlevels_a", nlevels_a);
            num_params_r__ += (1 * nlevels_a);
            current_statement_begin__ = 57;
            validate_non_negative_index("par2", "nlevels_BMD", nlevels_BMD);
            num_params_r__ += (1 * nlevels_BMD);
            current_statement_begin__ = 58;
            validate_non_negative_index("par3", "nlevels_d", nlevels_d);
            num_params_r__ += (1 * nlevels_d);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
            throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
        }
    }
    ~model_mG4_Q_COV() { }
    void transform_inits(const stan::io::var_context& context__,
                         std::vector<int>& params_i__,
                         std::vector<double>& params_r__,
                         std::ostream* pstream__) const {
        typedef double local_scalar_t__;
        stan::io::writer<double> writer__(params_r__, params_i__);
        size_t pos__;
        (void) pos__; // dummy call to supress warning
        std::vector<double> vals_r__;
        std::vector<int> vals_i__;
        current_statement_begin__ = 56;
        if (!(context__.contains_r("par1")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable par1 missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("par1");
        pos__ = 0U;
        validate_non_negative_index("par1", "nlevels_a", nlevels_a);
        context__.validate_dims("parameter initialization", "par1", "double", context__.to_vec(nlevels_a));
        std::vector<double> par1(nlevels_a, double(0));
        size_t par1_k_0_max__ = nlevels_a;
        for (size_t k_0__ = 0; k_0__ < par1_k_0_max__; ++k_0__) {
            par1[k_0__] = vals_r__[pos__++];
        }
        size_t par1_i_0_max__ = nlevels_a;
        for (size_t i_0__ = 0; i_0__ < par1_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_lub_unconstrain(0, 1, par1[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable par1: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 57;
        if (!(context__.contains_r("par2")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable par2 missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("par2");
        pos__ = 0U;
        validate_non_negative_index("par2", "nlevels_BMD", nlevels_BMD);
        context__.validate_dims("parameter initialization", "par2", "double", context__.to_vec(nlevels_BMD));
        std::vector<double> par2(nlevels_BMD, double(0));
        size_t par2_k_0_max__ = nlevels_BMD;
        for (size_t k_0__ = 0; k_0__ < par2_k_0_max__; ++k_0__) {
            par2[k_0__] = vals_r__[pos__++];
        }
        size_t par2_i_0_max__ = nlevels_BMD;
        for (size_t i_0__ = 0; i_0__ < par2_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_lb_unconstrain(0, par2[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable par2: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 58;
        if (!(context__.contains_r("par3")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable par3 missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("par3");
        pos__ = 0U;
        validate_non_negative_index("par3", "nlevels_d", nlevels_d);
        context__.validate_dims("parameter initialization", "par3", "double", context__.to_vec(nlevels_d));
        std::vector<double> par3(nlevels_d, double(0));
        size_t par3_k_0_max__ = nlevels_d;
        for (size_t k_0__ = 0; k_0__ < par3_k_0_max__; ++k_0__) {
            par3[k_0__] = vals_r__[pos__++];
        }
        size_t par3_i_0_max__ = nlevels_d;
        for (size_t i_0__ = 0; i_0__ < par3_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_unconstrain(par3[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable par3: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        params_r__ = writer__.data_r();
        params_i__ = writer__.data_i();
    }
    void transform_inits(const stan::io::var_context& context,
                         Eigen::Matrix<double, Eigen::Dynamic, 1>& params_r,
                         std::ostream* pstream__) const {
      std::vector<double> params_r_vec;
      std::vector<int> params_i_vec;
      transform_inits(context, params_i_vec, params_r_vec, pstream__);
      params_r.resize(params_r_vec.size());
      for (int i = 0; i < params_r.size(); ++i)
        params_r(i) = params_r_vec[i];
    }
    template <bool propto__, bool jacobian__, typename T__>
    T__ log_prob(std::vector<T__>& params_r__,
                 std::vector<int>& params_i__,
                 std::ostream* pstream__ = 0) const {
        typedef T__ local_scalar_t__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // dummy to suppress unused var warning
        T__ lp__(0.0);
        stan::math::accumulator<T__> lp_accum__;
        try {
            stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
            // model parameters
            current_statement_begin__ = 56;
            std::vector<local_scalar_t__> par1;
            size_t par1_d_0_max__ = nlevels_a;
            par1.reserve(par1_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < par1_d_0_max__; ++d_0__) {
                if (jacobian__)
                    par1.push_back(in__.scalar_lub_constrain(0, 1, lp__));
                else
                    par1.push_back(in__.scalar_lub_constrain(0, 1));
            }
            current_statement_begin__ = 57;
            std::vector<local_scalar_t__> par2;
            size_t par2_d_0_max__ = nlevels_BMD;
            par2.reserve(par2_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < par2_d_0_max__; ++d_0__) {
                if (jacobian__)
                    par2.push_back(in__.scalar_lb_constrain(0, lp__));
                else
                    par2.push_back(in__.scalar_lb_constrain(0));
            }
            current_statement_begin__ = 58;
            std::vector<local_scalar_t__> par3;
            size_t par3_d_0_max__ = nlevels_d;
            par3.reserve(par3_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < par3_d_0_max__; ++d_0__) {
                if (jacobian__)
                    par3.push_back(in__.scalar_constrain(lp__));
                else
                    par3.push_back(in__.scalar_constrain());
            }
            // transformed parameters
            current_statement_begin__ = 61;
            validate_non_negative_index("a", "nlevels_a", nlevels_a);
            std::vector<local_scalar_t__> a(nlevels_a, local_scalar_t__(0));
            stan::math::initialize(a, DUMMY_VAR__);
            stan::math::fill(a, DUMMY_VAR__);
            current_statement_begin__ = 62;
            validate_non_negative_index("b", "nlevels_b", nlevels_b);
            std::vector<local_scalar_t__> b(nlevels_b, local_scalar_t__(0));
            stan::math::initialize(b, DUMMY_VAR__);
            stan::math::fill(b, DUMMY_VAR__);
            current_statement_begin__ = 63;
            validate_non_negative_index("d", "nlevels_d", nlevels_d);
            std::vector<local_scalar_t__> d(nlevels_d, local_scalar_t__(0));
            stan::math::initialize(d, DUMMY_VAR__);
            stan::math::fill(d, DUMMY_VAR__);
            current_statement_begin__ = 64;
            validate_non_negative_index("k", "nlevels_BMD", nlevels_BMD);
            std::vector<local_scalar_t__> k(nlevels_BMD, local_scalar_t__(0));
            stan::math::initialize(k, DUMMY_VAR__);
            stan::math::fill(k, DUMMY_VAR__);
            current_statement_begin__ = 66;
            validate_non_negative_index("BMD", "nlevels_BMD", nlevels_BMD);
            std::vector<local_scalar_t__> BMD(nlevels_BMD, local_scalar_t__(0));
            stan::math::initialize(BMD, DUMMY_VAR__);
            stan::math::fill(BMD, DUMMY_VAR__);
            current_statement_begin__ = 67;
            validate_non_negative_index("theta", "2", 2);
            Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> theta(2);
            stan::math::initialize(theta, DUMMY_VAR__);
            stan::math::fill(theta, DUMMY_VAR__);
            current_statement_begin__ = 68;
            validate_non_negative_index("y_guess", "1", 1);
            Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> y_guess(1);
            stan::math::initialize(y_guess, DUMMY_VAR__);
            stan::math::fill(y_guess, DUMMY_VAR__);
            current_statement_begin__ = 69;
            validate_non_negative_index("yG", "1", 1);
            Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> yG(1);
            stan::math::initialize(yG, DUMMY_VAR__);
            stan::math::fill(yG, DUMMY_VAR__);
            // transformed parameters block statements
            current_statement_begin__ = 71;
            for (int mn = 1; mn <= nlevels_BMD; ++mn) {
                current_statement_begin__ = 72;
                stan::model::assign(BMD, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            get_base1(par2, mn, "par2", 1), 
                            "assigning variable BMD");
                current_statement_begin__ = 73;
                stan::model::assign(k, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            stan::math::log(get_base1(par2, mn, "par2", 1)), 
                            "assigning variable k");
            }
            current_statement_begin__ = 76;
            for (int mn = 1; mn <= nlevels_a; ++mn) {
                current_statement_begin__ = 77;
                stan::model::assign(a, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            get_base1(par1, mn, "par1", 1), 
                            "assigning variable a");
            }
            current_statement_begin__ = 80;
            for (int mn = 1; mn <= nlevels_d; ++mn) {
                current_statement_begin__ = 81;
                stan::model::assign(d, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            stan::math::exp(get_base1(par3, mn, "par3", 1)), 
                            "assigning variable d");
            }
            current_statement_begin__ = 84;
            if (as_bool((primitive_value((primitive_value(logical_eq(nlevels_a, 1)) && primitive_value(logical_eq(nlevels_d, 1)))) && primitive_value(logical_eq(nlevels_BMD, 1))))) {
                current_statement_begin__ = 85;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 86;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, 1, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 87;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, 1, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 88;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 89;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 90;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            } else if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_a, 1)) && primitive_value(logical_eq(nlevels_d, 1)))) && primitive_value(logical_eq(nlevels_BMD, 1))))) {
                current_statement_begin__ = 93;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 94;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, 1, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 95;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, 1, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 96;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 97;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 98;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            } else if (as_bool((primitive_value((primitive_value(logical_eq(nlevels_a, 1)) && primitive_value(logical_gt(nlevels_d, 1)))) && primitive_value(logical_gt(nlevels_BMD, 1))))) {
                current_statement_begin__ = 101;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 102;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, mn, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 103;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, mn, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 104;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 105;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 106;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            } else if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_a, 1)) && primitive_value(logical_gt(nlevels_d, 1)))) && primitive_value(logical_gt(nlevels_BMD, 1))))) {
                current_statement_begin__ = 109;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 110;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, mn, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 111;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, mn, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 112;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 113;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 114;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            }
            // validate transformed parameters
            const char* function__ = "validate transformed params";
            (void) function__;  // dummy to suppress unused var warning
            current_statement_begin__ = 61;
            size_t a_k_0_max__ = nlevels_a;
            for (size_t k_0__ = 0; k_0__ < a_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(a[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: a" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable a: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 62;
            size_t b_k_0_max__ = nlevels_b;
            for (size_t k_0__ = 0; k_0__ < b_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(b[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: b" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable b: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 63;
            size_t d_k_0_max__ = nlevels_d;
            for (size_t k_0__ = 0; k_0__ < d_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(d[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: d" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable d: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 64;
            size_t k_k_0_max__ = nlevels_BMD;
            for (size_t k_0__ = 0; k_0__ < k_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(k[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: k" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable k: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 66;
            size_t BMD_k_0_max__ = nlevels_BMD;
            for (size_t k_0__ = 0; k_0__ < BMD_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(BMD[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: BMD" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable BMD: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            size_t BMD_i_0_max__ = nlevels_BMD;
            for (size_t i_0__ = 0; i_0__ < BMD_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "BMD[i_0__]", BMD[i_0__], 0);
            }
            current_statement_begin__ = 67;
            size_t theta_j_1_max__ = 2;
            for (size_t j_1__ = 0; j_1__ < theta_j_1_max__; ++j_1__) {
                if (stan::math::is_uninitialized(theta(j_1__))) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: theta" << "(" << j_1__ << ")";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable theta: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 68;
            size_t y_guess_j_1_max__ = 1;
            for (size_t j_1__ = 0; j_1__ < y_guess_j_1_max__; ++j_1__) {
                if (stan::math::is_uninitialized(y_guess(j_1__))) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: y_guess" << "(" << j_1__ << ")";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable y_guess: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 69;
            size_t yG_j_1_max__ = 1;
            for (size_t j_1__ = 0; j_1__ < yG_j_1_max__; ++j_1__) {
                if (stan::math::is_uninitialized(yG(j_1__))) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: yG" << "(" << j_1__ << ")";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable yG: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            // model body
            current_statement_begin__ = 122;
            for (int i = 1; i <= nlevels_a; ++i) {
                current_statement_begin__ = 123;
                lp_accum__.add(pert_dist_lpdf<propto__>(get_base1(par1, i, "par1", 1), get_base1(priorlb, 1, i, "priorlb", 1), get_base1(priormu, 1, i, "priormu", 1), get_base1(priorub, 1, i, "priorub", 1), get_base1(priorgama, 1, i, "priorgama", 1), pstream__));
            }
            current_statement_begin__ = 127;
            for (int i = 1; i <= nlevels_BMD; ++i) {
                current_statement_begin__ = 128;
                lp_accum__.add(pert_dist_lpdf<propto__>(get_base1(par2, i, "par2", 1), get_base1(priorlb, 2, i, "priorlb", 1), get_base1(priormu, 2, i, "priormu", 1), get_base1(priorub, 2, i, "priorub", 1), get_base1(priorgama, 2, i, "priorgama", 1), pstream__));
            }
            current_statement_begin__ = 132;
            for (int i = 1; i <= nlevels_d; ++i) {
                current_statement_begin__ = 133;
                lp_accum__.add(normal_log<propto__>(get_base1(par3, i, "par3", 1), get_base1(priormu, 3, i, "priormu", 1), get_base1(priorSigma, 3, 3, "priorSigma", 1)));
                if (get_base1(par3, i, "par3", 1) > truncd) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-normal_cdf_log(truncd, get_base1(priormu, 3, i, "priormu", 1), get_base1(priorSigma, 3, 3, "priorSigma", 1)));
            }
            current_statement_begin__ = 136;
            if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_d, 1)) && primitive_value(logical_gt(nlevels_BMD, 1)))) && primitive_value(logical_eq(nlevels_a, 1))))) {
                current_statement_begin__ = 137;
                for (int i = 1; i <= N; ++i) {
                    current_statement_begin__ = 138;
                    for (int mn = 1; mn <= nlevels; ++mn) {
                        current_statement_begin__ = 139;
                        if (as_bool(logical_eq(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 139;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log((get_base1(a, 1, "a", 1) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - get_base1(a, 1, "a", 1)) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        } else if (as_bool(logical_gt(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 142;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log(((get_base1(a, 1, "a", 1) + ((1 - get_base1(a, 1, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, mn, "d", 1), get_base1(b, mn, "b", 1)))) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - (get_base1(a, 1, "a", 1) + ((1 - get_base1(a, 1, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, mn, "d", 1), get_base1(b, mn, "b", 1))))) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        }
                    }
                }
            } else if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_a, 1)) && primitive_value(logical_eq(nlevels_d, 1)))) && primitive_value(logical_eq(nlevels_BMD, 1))))) {
                current_statement_begin__ = 149;
                for (int i = 1; i <= N; ++i) {
                    current_statement_begin__ = 150;
                    for (int mn = 1; mn <= nlevels; ++mn) {
                        current_statement_begin__ = 151;
                        if (as_bool(logical_eq(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 151;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log((get_base1(a, mn, "a", 1) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - get_base1(a, mn, "a", 1)) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        } else if (as_bool(logical_gt(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 154;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log(((get_base1(a, mn, "a", 1) + ((1 - get_base1(a, mn, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, 1, "d", 1), get_base1(b, 1, "b", 1)))) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - (get_base1(a, mn, "a", 1) + ((1 - get_base1(a, mn, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, 1, "d", 1), get_base1(b, 1, "b", 1))))) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        }
                    }
                }
            } else if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_a, 1)) && primitive_value(logical_gt(nlevels_d, 1)))) && primitive_value(logical_gt(nlevels_BMD, 1))))) {
                current_statement_begin__ = 161;
                for (int i = 1; i <= N; ++i) {
                    current_statement_begin__ = 162;
                    for (int mn = 1; mn <= nlevels; ++mn) {
                        current_statement_begin__ = 163;
                        if (as_bool(logical_eq(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 163;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log((get_base1(a, mn, "a", 1) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - get_base1(a, mn, "a", 1)) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        } else if (as_bool(logical_gt(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 166;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log(((get_base1(a, mn, "a", 1) + ((1 - get_base1(a, mn, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, mn, "d", 1), get_base1(b, mn, "b", 1)))) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - (get_base1(a, mn, "a", 1) + ((1 - get_base1(a, mn, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, mn, "d", 1), get_base1(b, mn, "b", 1))))) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        }
                    }
                }
            } else if (as_bool((primitive_value((primitive_value(logical_eq(nlevels_a, 1)) && primitive_value(logical_eq(nlevels_d, 1)))) && primitive_value(logical_eq(nlevels_BMD, 1))))) {
                current_statement_begin__ = 173;
                for (int i = 1; i <= N; ++i) {
                    current_statement_begin__ = 174;
                    for (int mn = 1; mn <= nlevels; ++mn) {
                        current_statement_begin__ = 175;
                        if (as_bool(logical_eq(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 175;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log((get_base1(a, 1, "a", 1) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - get_base1(a, 1, "a", 1)) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        } else if (as_bool(logical_gt(get_base1(x, i, "x", 1), 0))) {
                            current_statement_begin__ = 178;
                            lp_accum__.add((((binomial_coefficient_log(get_base1(n, i, "n", 1), get_base1(y, i, "y", 1)) + (get_base1(y, i, "y", 1) * stan::math::log(((get_base1(a, 1, "a", 1) + ((1 - get_base1(a, 1, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, 1, "d", 1), get_base1(b, 1, "b", 1)))) + eps)))) + ((get_base1(n, i, "n", 1) - get_base1(y, i, "y", 1)) * stan::math::log(((1 - (get_base1(a, 1, "a", 1) + ((1 - get_base1(a, 1, "a", 1)) * gamma_cdf(get_base1(x, i, "x", 1), get_base1(d, 1, "d", 1), get_base1(b, 1, "b", 1))))) + eps)))) * get_base1(trt_ind, i, mn, "trt_ind", 1)));
                        }
                    }
                }
            }
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
            throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
        }
        lp_accum__.add(lp__);
        return lp_accum__.sum();
    } // log_prob()
    template <bool propto, bool jacobian, typename T_>
    T_ log_prob(Eigen::Matrix<T_,Eigen::Dynamic,1>& params_r,
               std::ostream* pstream = 0) const {
      std::vector<T_> vec_params_r;
      vec_params_r.reserve(params_r.size());
      for (int i = 0; i < params_r.size(); ++i)
        vec_params_r.push_back(params_r(i));
      std::vector<int> vec_params_i;
      return log_prob<propto,jacobian,T_>(vec_params_r, vec_params_i, pstream);
    }
    void get_param_names(std::vector<std::string>& names__) const {
        names__.resize(0);
        names__.push_back("par1");
        names__.push_back("par2");
        names__.push_back("par3");
        names__.push_back("a");
        names__.push_back("b");
        names__.push_back("d");
        names__.push_back("k");
        names__.push_back("BMD");
        names__.push_back("theta");
        names__.push_back("y_guess");
        names__.push_back("yG");
    }
    void get_dims(std::vector<std::vector<size_t> >& dimss__) const {
        dimss__.resize(0);
        std::vector<size_t> dims__;
        dims__.resize(0);
        dims__.push_back(nlevels_a);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_BMD);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_d);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_a);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_b);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_d);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_BMD);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(nlevels_BMD);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(2);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(1);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(1);
        dimss__.push_back(dims__);
    }
    template <typename RNG>
    void write_array(RNG& base_rng__,
                     std::vector<double>& params_r__,
                     std::vector<int>& params_i__,
                     std::vector<double>& vars__,
                     bool include_tparams__ = true,
                     bool include_gqs__ = true,
                     std::ostream* pstream__ = 0) const {
        typedef double local_scalar_t__;
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
        static const char* function__ = "model_mG4_Q_COV_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
        // read-transform, write parameters
        std::vector<double> par1;
        size_t par1_d_0_max__ = nlevels_a;
        par1.reserve(par1_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < par1_d_0_max__; ++d_0__) {
            par1.push_back(in__.scalar_lub_constrain(0, 1));
        }
        size_t par1_k_0_max__ = nlevels_a;
        for (size_t k_0__ = 0; k_0__ < par1_k_0_max__; ++k_0__) {
            vars__.push_back(par1[k_0__]);
        }
        std::vector<double> par2;
        size_t par2_d_0_max__ = nlevels_BMD;
        par2.reserve(par2_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < par2_d_0_max__; ++d_0__) {
            par2.push_back(in__.scalar_lb_constrain(0));
        }
        size_t par2_k_0_max__ = nlevels_BMD;
        for (size_t k_0__ = 0; k_0__ < par2_k_0_max__; ++k_0__) {
            vars__.push_back(par2[k_0__]);
        }
        std::vector<double> par3;
        size_t par3_d_0_max__ = nlevels_d;
        par3.reserve(par3_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < par3_d_0_max__; ++d_0__) {
            par3.push_back(in__.scalar_constrain());
        }
        size_t par3_k_0_max__ = nlevels_d;
        for (size_t k_0__ = 0; k_0__ < par3_k_0_max__; ++k_0__) {
            vars__.push_back(par3[k_0__]);
        }
        double lp__ = 0.0;
        (void) lp__;  // dummy to suppress unused var warning
        stan::math::accumulator<double> lp_accum__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
        if (!include_tparams__ && !include_gqs__) return;
        try {
            // declare and define transformed parameters
            current_statement_begin__ = 61;
            validate_non_negative_index("a", "nlevels_a", nlevels_a);
            std::vector<double> a(nlevels_a, double(0));
            stan::math::initialize(a, DUMMY_VAR__);
            stan::math::fill(a, DUMMY_VAR__);
            current_statement_begin__ = 62;
            validate_non_negative_index("b", "nlevels_b", nlevels_b);
            std::vector<double> b(nlevels_b, double(0));
            stan::math::initialize(b, DUMMY_VAR__);
            stan::math::fill(b, DUMMY_VAR__);
            current_statement_begin__ = 63;
            validate_non_negative_index("d", "nlevels_d", nlevels_d);
            std::vector<double> d(nlevels_d, double(0));
            stan::math::initialize(d, DUMMY_VAR__);
            stan::math::fill(d, DUMMY_VAR__);
            current_statement_begin__ = 64;
            validate_non_negative_index("k", "nlevels_BMD", nlevels_BMD);
            std::vector<double> k(nlevels_BMD, double(0));
            stan::math::initialize(k, DUMMY_VAR__);
            stan::math::fill(k, DUMMY_VAR__);
            current_statement_begin__ = 66;
            validate_non_negative_index("BMD", "nlevels_BMD", nlevels_BMD);
            std::vector<double> BMD(nlevels_BMD, double(0));
            stan::math::initialize(BMD, DUMMY_VAR__);
            stan::math::fill(BMD, DUMMY_VAR__);
            current_statement_begin__ = 67;
            validate_non_negative_index("theta", "2", 2);
            Eigen::Matrix<double, Eigen::Dynamic, 1> theta(2);
            stan::math::initialize(theta, DUMMY_VAR__);
            stan::math::fill(theta, DUMMY_VAR__);
            current_statement_begin__ = 68;
            validate_non_negative_index("y_guess", "1", 1);
            Eigen::Matrix<double, Eigen::Dynamic, 1> y_guess(1);
            stan::math::initialize(y_guess, DUMMY_VAR__);
            stan::math::fill(y_guess, DUMMY_VAR__);
            current_statement_begin__ = 69;
            validate_non_negative_index("yG", "1", 1);
            Eigen::Matrix<double, Eigen::Dynamic, 1> yG(1);
            stan::math::initialize(yG, DUMMY_VAR__);
            stan::math::fill(yG, DUMMY_VAR__);
            // do transformed parameters statements
            current_statement_begin__ = 71;
            for (int mn = 1; mn <= nlevels_BMD; ++mn) {
                current_statement_begin__ = 72;
                stan::model::assign(BMD, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            get_base1(par2, mn, "par2", 1), 
                            "assigning variable BMD");
                current_statement_begin__ = 73;
                stan::model::assign(k, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            stan::math::log(get_base1(par2, mn, "par2", 1)), 
                            "assigning variable k");
            }
            current_statement_begin__ = 76;
            for (int mn = 1; mn <= nlevels_a; ++mn) {
                current_statement_begin__ = 77;
                stan::model::assign(a, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            get_base1(par1, mn, "par1", 1), 
                            "assigning variable a");
            }
            current_statement_begin__ = 80;
            for (int mn = 1; mn <= nlevels_d; ++mn) {
                current_statement_begin__ = 81;
                stan::model::assign(d, 
                            stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                            stan::math::exp(get_base1(par3, mn, "par3", 1)), 
                            "assigning variable d");
            }
            current_statement_begin__ = 84;
            if (as_bool((primitive_value((primitive_value(logical_eq(nlevels_a, 1)) && primitive_value(logical_eq(nlevels_d, 1)))) && primitive_value(logical_eq(nlevels_BMD, 1))))) {
                current_statement_begin__ = 85;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 86;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, 1, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 87;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, 1, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 88;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 89;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 90;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            } else if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_a, 1)) && primitive_value(logical_eq(nlevels_d, 1)))) && primitive_value(logical_eq(nlevels_BMD, 1))))) {
                current_statement_begin__ = 93;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 94;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, 1, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 95;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, 1, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 96;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 97;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 98;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            } else if (as_bool((primitive_value((primitive_value(logical_eq(nlevels_a, 1)) && primitive_value(logical_gt(nlevels_d, 1)))) && primitive_value(logical_gt(nlevels_BMD, 1))))) {
                current_statement_begin__ = 101;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 102;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, mn, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 103;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, mn, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 104;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 105;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 106;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            } else if (as_bool((primitive_value((primitive_value(logical_gt(nlevels_a, 1)) && primitive_value(logical_gt(nlevels_d, 1)))) && primitive_value(logical_gt(nlevels_BMD, 1))))) {
                current_statement_begin__ = 109;
                for (int mn = 1; mn <= nlevels_b; ++mn) {
                    current_statement_begin__ = 110;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                get_base1(BMD, mn, "BMD", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 111;
                    stan::model::assign(theta, 
                                stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list()), 
                                get_base1(d, mn, "d", 1), 
                                "assigning variable theta");
                    current_statement_begin__ = 112;
                    stan::model::assign(y_guess, 
                                stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list()), 
                                init_b, 
                                "assigning variable y_guess");
                    current_statement_begin__ = 113;
                    stan::math::assign(yG, algebra_solver(algebra_system_functor__(), y_guess, theta, x_r, x_i, pstream__, 1e-10, stan::math::positive_infinity(), 1e3));
                    current_statement_begin__ = 114;
                    stan::model::assign(b, 
                                stan::model::cons_list(stan::model::index_uni(mn), stan::model::nil_index_list()), 
                                get_base1(yG, 1, "yG", 1), 
                                "assigning variable b");
                }
            }
            if (!include_gqs__ && !include_tparams__) return;
            // validate transformed parameters
            const char* function__ = "validate transformed params";
            (void) function__;  // dummy to suppress unused var warning
            current_statement_begin__ = 66;
            size_t BMD_i_0_max__ = nlevels_BMD;
            for (size_t i_0__ = 0; i_0__ < BMD_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "BMD[i_0__]", BMD[i_0__], 0);
            }
            // write transformed parameters
            if (include_tparams__) {
                size_t a_k_0_max__ = nlevels_a;
                for (size_t k_0__ = 0; k_0__ < a_k_0_max__; ++k_0__) {
                    vars__.push_back(a[k_0__]);
                }
                size_t b_k_0_max__ = nlevels_b;
                for (size_t k_0__ = 0; k_0__ < b_k_0_max__; ++k_0__) {
                    vars__.push_back(b[k_0__]);
                }
                size_t d_k_0_max__ = nlevels_d;
                for (size_t k_0__ = 0; k_0__ < d_k_0_max__; ++k_0__) {
                    vars__.push_back(d[k_0__]);
                }
                size_t k_k_0_max__ = nlevels_BMD;
                for (size_t k_0__ = 0; k_0__ < k_k_0_max__; ++k_0__) {
                    vars__.push_back(k[k_0__]);
                }
                size_t BMD_k_0_max__ = nlevels_BMD;
                for (size_t k_0__ = 0; k_0__ < BMD_k_0_max__; ++k_0__) {
                    vars__.push_back(BMD[k_0__]);
                }
                size_t theta_j_1_max__ = 2;
                for (size_t j_1__ = 0; j_1__ < theta_j_1_max__; ++j_1__) {
                    vars__.push_back(theta(j_1__));
                }
                size_t y_guess_j_1_max__ = 1;
                for (size_t j_1__ = 0; j_1__ < y_guess_j_1_max__; ++j_1__) {
                    vars__.push_back(y_guess(j_1__));
                }
                size_t yG_j_1_max__ = 1;
                for (size_t j_1__ = 0; j_1__ < yG_j_1_max__; ++j_1__) {
                    vars__.push_back(yG(j_1__));
                }
            }
            if (!include_gqs__) return;
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
            throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
        }
    }
    template <typename RNG>
    void write_array(RNG& base_rng,
                     Eigen::Matrix<double,Eigen::Dynamic,1>& params_r,
                     Eigen::Matrix<double,Eigen::Dynamic,1>& vars,
                     bool include_tparams = true,
                     bool include_gqs = true,
                     std::ostream* pstream = 0) const {
      std::vector<double> params_r_vec(params_r.size());
      for (int i = 0; i < params_r.size(); ++i)
        params_r_vec[i] = params_r(i);
      std::vector<double> vars_vec;
      std::vector<int> params_i_vec;
      write_array(base_rng, params_r_vec, params_i_vec, vars_vec, include_tparams, include_gqs, pstream);
      vars.resize(vars_vec.size());
      for (int i = 0; i < vars.size(); ++i)
        vars(i) = vars_vec[i];
    }
    std::string model_name() const {
        return "model_mG4_Q_COV";
    }
    void constrained_param_names(std::vector<std::string>& param_names__,
                                 bool include_tparams__ = true,
                                 bool include_gqs__ = true) const {
        std::stringstream param_name_stream__;
        size_t par1_k_0_max__ = nlevels_a;
        for (size_t k_0__ = 0; k_0__ < par1_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "par1" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t par2_k_0_max__ = nlevels_BMD;
        for (size_t k_0__ = 0; k_0__ < par2_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "par2" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t par3_k_0_max__ = nlevels_d;
        for (size_t k_0__ = 0; k_0__ < par3_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "par3" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        if (!include_gqs__ && !include_tparams__) return;
        if (include_tparams__) {
            size_t a_k_0_max__ = nlevels_a;
            for (size_t k_0__ = 0; k_0__ < a_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "a" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t b_k_0_max__ = nlevels_b;
            for (size_t k_0__ = 0; k_0__ < b_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "b" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t d_k_0_max__ = nlevels_d;
            for (size_t k_0__ = 0; k_0__ < d_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "d" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t k_k_0_max__ = nlevels_BMD;
            for (size_t k_0__ = 0; k_0__ < k_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "k" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t BMD_k_0_max__ = nlevels_BMD;
            for (size_t k_0__ = 0; k_0__ < BMD_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "BMD" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t theta_j_1_max__ = 2;
            for (size_t j_1__ = 0; j_1__ < theta_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "theta" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t y_guess_j_1_max__ = 1;
            for (size_t j_1__ = 0; j_1__ < y_guess_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "y_guess" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t yG_j_1_max__ = 1;
            for (size_t j_1__ = 0; j_1__ < yG_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "yG" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        if (!include_gqs__) return;
    }
    void unconstrained_param_names(std::vector<std::string>& param_names__,
                                   bool include_tparams__ = true,
                                   bool include_gqs__ = true) const {
        std::stringstream param_name_stream__;
        size_t par1_k_0_max__ = nlevels_a;
        for (size_t k_0__ = 0; k_0__ < par1_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "par1" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t par2_k_0_max__ = nlevels_BMD;
        for (size_t k_0__ = 0; k_0__ < par2_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "par2" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t par3_k_0_max__ = nlevels_d;
        for (size_t k_0__ = 0; k_0__ < par3_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "par3" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        if (!include_gqs__ && !include_tparams__) return;
        if (include_tparams__) {
            size_t a_k_0_max__ = nlevels_a;
            for (size_t k_0__ = 0; k_0__ < a_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "a" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t b_k_0_max__ = nlevels_b;
            for (size_t k_0__ = 0; k_0__ < b_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "b" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t d_k_0_max__ = nlevels_d;
            for (size_t k_0__ = 0; k_0__ < d_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "d" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t k_k_0_max__ = nlevels_BMD;
            for (size_t k_0__ = 0; k_0__ < k_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "k" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t BMD_k_0_max__ = nlevels_BMD;
            for (size_t k_0__ = 0; k_0__ < BMD_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "BMD" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t theta_j_1_max__ = 2;
            for (size_t j_1__ = 0; j_1__ < theta_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "theta" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t y_guess_j_1_max__ = 1;
            for (size_t j_1__ = 0; j_1__ < y_guess_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "y_guess" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t yG_j_1_max__ = 1;
            for (size_t j_1__ = 0; j_1__ < yG_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "yG" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        if (!include_gqs__) return;
    }
}; // model
}  // namespace
typedef model_mG4_Q_COV_namespace::model_mG4_Q_COV stan_model;
#ifndef USING_R
stan::model::model_base& new_model(
        stan::io::var_context& data_context,
        unsigned int seed,
        std::ostream* msg_stream) {
  stan_model* m = new stan_model(data_context, seed, msg_stream);
  return *m;
}
#endif
#endif
