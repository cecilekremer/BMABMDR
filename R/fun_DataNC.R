...
PREP_DATA_N_C <- function(data, # a dataframe with input data, order of columns should be: dose, response, litter
  ...
  # b.test.N <- bartlett.test(indiv.data$response, indiv.data$dose)
  # # b.test.N <- bartlett(sd.a, n.a)
  # if(b.test.N$p.value>=0.05){
  #   test.var = paste0('Distributional assumption of constant variance are met, Bartlett test p-value is ', round(b.test.N$p.value, 4))
  #   message(test.var)
  ...
  if(norm.test.N$p.value>=0.05){
    test.var = paste0('Distributional assumption of normality of residuals for the normal distribution is met, Shapiro test p-value is ',
                      round(norm.test.N$p.value, 4))
    message(test.var)
  }else if(norm.test.N$p.value<0.05){
    test.var = paste0('Distributional assumption of normality of residuals for the normal distribution is not met, Shapiro test p-value is ',
                      round(norm.test.N$p.value, 4))
    warning(test.var)
  }
