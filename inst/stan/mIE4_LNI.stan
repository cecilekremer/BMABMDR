...
data{
  ...
  vector[N] m;  // the geometric mean of the response values for each dose group
  vector[N] s2;  // the geometric variance of the response values for each dose group
  real q;       // the BMR
  vector[5] priormu;
  vector[5] priorlb;
  vector[5] priorub;
...
parameters{
  vector[5] par; // a=a, BMD on log-scale, ct=log(c-1), dt=log(d), invsigma2t=log(invsigma2)
}
