...
transformed parameters{
  ...
  if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=-exp(-k[1]*d[1])*log(1-q/(c[1]-1));
      }else if(data_type == 2){
        b[mn]=-exp(-k[1]*d[1])*log(1-(log(1+q)/(a[1]*(c[1]-1))));
      }else if(data_type == 3){
        b[mn]=-exp(-k[1]*d[1])*log(1+q/(c[1]-1));
      }else if(data_type == 4){
        b[mn]=-exp(-k[1]*d[1])*log(1-(log(1-q)/(a[1]*(c[1]-1))));
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=-exp(-k[1]*d[1])*log(1-q/(c[mn]-1));
      }else if(data_type == 2){
        b[mn]=-exp(-k[1]*d[1])*log(1-(log(1+q)/(a[mn]*(c[mn]-1))));
      }else if(data_type == 3){
        b[mn]=-exp(-k[1]*d[1])*log(1+q/(c[mn]-1));
      }else if(data_type == 4){
        b[mn]=-exp(-k[1]*d[1])*log(1-(log(1-q)/(a[mn]*(c[mn]-1))));
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=-exp(-k[mn]*d[mn])*log(1-q/(c[1]-1));
      }else if(data_type == 2){
        b[mn]=-exp(-k[mn]*d[mn])*log(1-(log(1+q)/(a[1]*(c[1]-1))));
      }else if(data_type == 3){
        b[mn]=-exp(-k[mn]*d[mn])*log(1+q/(c[1]-1));
      }else if(data_type == 4){
        b[mn]=-exp(-k[mn]*d[mn])*log(1-(log(1-q)/(a[1]*(c[1]-1))));
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=-exp(-k[mn]*d[mn])*log(1-q/(c[mn]-1));
      }else if(data_type == 2){
        b[mn]=-exp(-k[mn]*d[mn])*log(1-(log(1+q)/(a[mn]*(c[mn]-1))));
      }else if(data_type == 3){
        b[mn]=-exp(-k[mn]*d[mn])*log(1+q/(c[mn]-1));
      }else if(data_type == 4){
        b[mn]=-exp(-k[mn]*d[mn])*log(1-(log(1-q)/(a[mn]*(c[mn]-1))));
  ...
  if(nlevels_d > 1 && nlevels_BMD > 1 && nlevels_a == 1 && nlevels_sigma == 1){
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          ...
          0.5*n[i]*square(m[i]-a[1]-a[1]*(c[1]-1)*(1-exp(-b[mn]*pow(x[i],d[mn]))))*
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          ...
          0.5*n[i]*square(m[i]-a[mn]-a[mn]*(c[mn]-1)*(1-exp(-b[mn]*pow(x[i],d[1]))))*
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[mn])-
          0.5*(n[i]-1)*s2[i]*invsigma2[mn]-
          0.5*n[i]*square(m[i]-a[mn]-a[mn]*(c[mn]-1)*(1-exp(-b[mn]*pow(x[i],d[mn]))))*
          invsigma2[mn])*trt_ind[i,mn];
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        ...
        target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
        0.5*(n[i]-1)*s2[i]*invsigma2[1]-
        0.5*n[i]*square(m[i]-a[1]-a[1]*(c[1]-1)*(1-exp(-b[1]*pow(x[i],d[1]))))*invsigma2[1])*trt_ind[i,mn];
        }
      }
    } else if(data_type == 2 || data_type == 4){
      for (i in 1:N){
        ...
        target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
        0.5*(n[i]-1)*s2[i]*invsigma2[1]-
        0.5*n[i]*square(m[i]-a[1]-a[1]*(c[1]-1)*(1-exp(-b[1]*pow(x[i],d[1]))))*invsigma2[1] - m[i]*n[i])*trt_ind[i,mn];
        }
      }
    }
