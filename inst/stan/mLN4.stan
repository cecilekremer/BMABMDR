...
transformed parameters{
  ...
  if(data_type == 1){
    b=exp(inv_Phi(q/(c-1))-(k*d));
  }else if(data_type == 2){
    b=exp(inv_Phi((log(1+q))/(a*(c-1)))-(k*d));
  }else if(data_type == 3){
    b=exp(inv_Phi(-q/(c-1))-(k*d));
  }else if(data_type == 4){
    b=exp(inv_Phi((log(1-q))/(a*(c-1)))-(k*d));
...
model{
 ...
 if(data_type == 1 || data_type == 3){
   for (i in 1:N){
     if (x[i]==0){target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-a)*invsigma2;}
     if (x[i]>0) {target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-a-a*(c-1)*Phi(log(b)+(d*log(x[i]))))*invsigma2;}
   }
 }else if(data_type == 2 || data_type == 4){
    for (i in 1:N){
     if (x[i]==0){target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-a)*invsigma2 - m[i]*n[i];}
     if (x[i]>0) {target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-a-a*(c-1)*Phi(log(b)+(d*log(x[i]))))*invsigma2 - m[i]*n[i];}
   }
 }
