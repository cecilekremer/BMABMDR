...
data{
  int N;  // the total number of distinct dose x covariate group
  // int N2;
...
transformed parameters{
  ...
  for(mn in 1:nlevels_a){
    ...
    if(data_type == 1 || data_type == 2){
      if(data_type == 1){
        ...
        a[mn] = log(mu_inf[mn]) - shift;
      }
    }else if(data_type == 3 || data_type == 4){
      if(data_type == 3){
        a[mn] = mu_0[mn];
      }else if(data_type == 4){
        a[mn] = log(mu_0[mn]) - shift;
      }
    }
  ...
  if(data_type == 1 || data_type == 2){
    if(data_type == 1){
      c[mn] = pow(3,0.5)/pi()*logit(mu_0[mn]/mu_inf[mn]);
    }else if(data_type == 2){
      c[mn] = pow(3,0.5)/pi()*logit((log(mu_0[mn])-shift)/(log(mu_inf[mn])-shift));
    }
  }else if(data_type == 3 || data_type == 4){
    if(data_type == 3){
      c[mn] = pow(3,0.5)/pi()*logit(mu_inf[mn]/mu_0[mn]);
    }else if(data_type == 4){
      c[mn] = pow(3,0.5)/pi()*logit((log(mu_inf[mn])-shift)/(log(mu_0[mn])-shift));
    }
  }
  ...
  if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[1]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])*(1+q))-c[1]);
      }else if(data_type == 2){
        b[1]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])+log(1+q)/a[1])-c[1]);
      }else if(data_type == 3){
        b[1]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])+q)-c[1]);
      }else if(data_type == 4){
        b[1]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])-log(1-q)/a[1])-c[1]);
      }
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])*(1+q))-c[mn]);
      }else if(data_type == 2){
        b[mn]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])+log(1+q)/a[mn])-c[mn]);
      }else if(data_type == 3){
        b[mn]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])+q)-c[mn]);
      }else if(data_type == 4){
        b[mn]=exp(-k[1]*d[1])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])-log(1-q)/a[mn])-c[mn]);
      }
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])*(1+q))-c[1]);
      }else if(data_type == 2){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])+log(1+q)/a[1])-c[1]);
      }else if(data_type == 3){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])+q)-c[1]);
      }else if(data_type == 4){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[1])-log(1-q)/a[1])-c[1]);
      }
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])*(1+q))-c[mn]);
      }else if(data_type == 2){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])+log(1+q)/a[mn])-c[mn]);
      }else if(data_type == 3){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])+q)-c[mn]);
      }else if(data_type == 4){
        b[mn]=exp(-k[mn]*d[mn])*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c[mn])-log(1-q)/a[mn])-c[mn]);
      }
    ...
    if(data_type == 1){
      for (i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
                       ...
                       0.5*n[i]*square(m[i]-(a[1]*inv_logit(pi()/pow(3,0.5)*(c[1]+b[mn]*pow(x[i],d[mn])))))*
      ...
      for(i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
                       0.5*(n[i]-1)*s2[i]*invsigma2[1]-
                       0.5*n[i]*square(m[i]-((a[1]*(1+inv_logit(pi()/pow(3,0.5)*c[1])))-(a[1]*inv_logit(pi()/pow(3,0.5)*(c[1]+b[mn]*pow(x[i],d[mn]))))))*
                       invsigma2[1])*trt_ind[i,mn];
        }
      }
    ...
    if(data_type == 1){
      for (i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[mn])-
                       ...
                       0.5*n[i]*square(m[i]-(a[mn]*inv_logit(pi()/pow(3,0.5)*(c[mn]+b[mn]*pow(x[i],d[1])))))*
      ...
      for(i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[mn])-
                       0.5*(n[i]-1)*s2[i]*invsigma2[mn]-
                       0.5*n[i]*square(m[i]-((a[mn]*(1+inv_logit(pi()/pow(3,0.5)*c[1])))-(a[mn]*inv_logit(pi()/pow(3,0.5)*(c[mn]+b[mn]*pow(x[i],d[1]))))))*
                       invsigma2[mn])*trt_ind[i,mn];
        }
      }
    ...
    if(data_type == 1){
      for (i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[mn])-
                       0.5*(n[i]-1)*s2[i]*invsigma2[mn]-
                       0.5*n[i]*square(m[i]-(a[mn]*inv_logit(pi()/pow(3,0.5)*(c[mn]+b[mn]*pow(x[i],d[mn])))))*
                       invsigma2[mn])*trt_ind[i,mn];
        }
      }
      ...
      for(i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[mn])-
                       0.5*(n[i]-1)*s2[i]*invsigma2[mn]-
                       0.5*n[i]*square(m[i]-((a[mn]*(1+inv_logit(pi()/pow(3,0.5)*c[1])))-(a[mn]*inv_logit(pi()/pow(3,0.5)*(c[mn]+b[mn]*pow(x[i],d[mn]))))))*
                       invsigma2[mn])*trt_ind[i,mn];
        }
      }
    ...
    if(data_type == 1){
      for (i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
                       0.5*(n[i]-1)*s2[i]*invsigma2[1]-
                       0.5*n[i]*square(m[i]-(a[1]*inv_logit(pi()/pow(3,0.5)*(c[1]+b[1]*pow(x[i],d[1])))))*
                       invsigma2[1])*trt_ind[i,mn];
        }
      }
      ...
      for(i in 1:N){
        for(mn in 1:nlevels){
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
                       0.5*(n[i]-1)*s2[i]*invsigma2[1]-
                       0.5*n[i]*square(m[i]-((a[1]*(1+inv_logit(pi()/pow(3,0.5)*c[1])))-(a[1]*inv_logit(pi()/pow(3,0.5)*(c[1]+b[1]*pow(x[i],d[1]))))))*
                       invsigma2[1])*trt_ind[i,mn];
        }
      }
