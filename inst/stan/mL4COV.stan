  ...
  for (mn in 1 : nlevels_a) {
    if (data_type == 1 || data_type == 2) {
      if (data_type == 1) {
        ...
        c[mn] = pow(3, 0.5) / pi()
                * logit((log(mu_0[mn]) - shift) / (log(mu_inf[mn]) - shift));
      }
    } else if (data_type == 3 || data_type == 4) {
      if (data_type == 3) {
        ...
        c[mn] = pow(3, 0.5) / pi()
                * logit((log(mu_inf[mn]) - shift) / (log(mu_0[mn]) - shift));
      }
    }
  }
  ...
  if (nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        b[1] = exp(-k[1] * d[1])
               * (pow(3, 0.5) / pi()
                  * logit(inv_logit(pi() / pow(3, 0.5) * c[1]) * (1 + q))
                  - c[1]);
      } else if (data_type == 2) {
        b[1] = exp(-k[1] * d[1])
               * (pow(3, 0.5) / pi()
                  * logit(
                          inv_logit(pi() / pow(3, 0.5) * c[1])
                          + log(1 + q) / a[1])
                  - c[1]);
      } else if (data_type == 3) {
        b[1] = exp(-k[1] * d[1])
               * (pow(3, 0.5) / pi()
                  * logit(inv_logit(pi() / pow(3, 0.5) * c[1]) + q) - c[1]);
      } else if (data_type == 4) {
        b[1] = exp(-k[1] * d[1])
               * (pow(3, 0.5) / pi()
                  * logit(
    ...
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        b[mn] = exp(-k[1] * d[1])
                * (pow(3, 0.5) / pi()
                   * logit(inv_logit(pi() / pow(3, 0.5) * c[mn]) * (1 + q))
                   - c[mn]);
      } else if (data_type == 2) {
        b[mn] = exp(-k[1] * d[1])
                * (pow(3, 0.5) / pi()
                   * logit(
                           inv_logit(pi() / pow(3, 0.5) * c[mn])
                           + log(1 + q) / a[mn])
                   - c[mn]);
      } else if (data_type == 3) {
        b[mn] = exp(-k[1] * d[1])
                * (pow(3, 0.5) / pi()
                   * logit(inv_logit(pi() / pow(3, 0.5) * c[mn]) + q) - c[mn]);
      } else if (data_type == 4) {
        b[mn] = exp(-k[1] * d[1])
                * (pow(3, 0.5) / pi()
                   * logit(
