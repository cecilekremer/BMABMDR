...
transformed parameters{
  ...
  a=exp(par[1]);
  //c=par[3];
  c=inv_Phi(1+(log(1-q)/a)) - exp(par[3]);
  d=exp(par[4]);
  b=exp(-par[2]*d)*(inv_Phi(Phi(c)-log(1-q)/a)-c);
  invsigma2=exp(par[5]);
}
model{
   ...
   for (i in 1:N){
     target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-((a*(1+Phi(c)))-(a*Phi(c+b*pow(x[i],d)))))*invsigma2 - m[i]*n[i];
   }
}
