...
transformed parameters{
  ...
  real invsigma2;
  a=exp(par[1]);
  c=pow(3,0.5)/pi()*logit(1/(1+q)) - exp(par[3]);
  //c=par[3];
  d=exp(par[4]);
  b=exp(-par[2]*d)*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c)*(1+q))-c);
  invsigma2=exp(par[5]);
}
model{
  ...
  par[5] ~ normal(priormu[5],priorSigma[5,5]);
   ...
   for (i in 1:N){
     target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-(a*inv_logit(pi()/pow(3,0.5)*(c+b*pow(x[i],d)))))*invsigma2;
   }
}
