...
transformed parameters {
  ...
  if (data_type == 1 || data_type == 2) {
    if (data_type == 1) {
      ...
      c = pow(3, 0.5) / pi()
          * logit((log(mu_0) - shift) / (log(mu_inf) - shift));
    }
    ...
    if (data_type == 3) {
      ...
      c = pow(3, 0.5) / pi()
          * logit((log(mu_inf) - shift) / (log(mu_0) - shift));
    }
  }
  ...
  if (data_type == 1) {
    b = exp(-k * d)
        * (pow(3, 0.5) / pi()
           * logit(inv_logit(pi() / pow(3, 0.5) * c) * (1 + q)) - c);
  } else if (data_type == 2) {
    b = exp(-k * d)
        * (pow(3, 0.5) / pi()
           * logit(inv_logit(pi() / pow(3, 0.5) * c) + log(1 + q) / a) - c);
  } else if (data_type == 3) {
    b = exp(-k * d)
        * (pow(3, 0.5) / pi() * logit(inv_logit(pi() / pow(3, 0.5) * c) + q)
           - c);
  } else if (data_type == 4) {
    b = exp(-k * d)
        * (pow(3, 0.5) / pi()
