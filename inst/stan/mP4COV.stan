  ...
  if (nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        ...
        b[1] = exp(-k[1] * d[1])
               * (inv_Phi(Phi(c[1]) + log(1 + q) / a[1]) - c[1]);
      } else if (data_type == 3) {
    ...
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        ...
        b[mn] = exp(-k[1] * d[1])
                * (inv_Phi(Phi(c[mn]) + log(1 + q) / a[mn]) - c[mn]);
      } else if (data_type == 3) {
...
model {
  ...
  for (i in 1 : nlevels_a) {
    par1[i] ~ pert_dist(shape1[1, i], shape2[1, i], priorlb[1, i],
                        priorub[1, i]);
  }
  
  //BMD prior
  for (i in 1 : nlevels_BMD) {
    par2[i] ~ pert_dist(shape1[2, i], shape2[2, i], priorlb[2, i],
                        priorub[2, i]);
  }
  ...
  if (nlevels_d > 1 && nlevels_BMD > 1 && nlevels_a == 1
    ...
    if (data_type == 1) {
      for (i in 1 : N) {
        for (mn in 1 : nlevels) {
          target += (-0.5 * n[i] * log(2 * pi())
                     + 0.5 * n[i] * log(invsigma2[1])
                     - 0.5 * (n[i] - 1) * s2[i] * invsigma2[1]
                     - 0.5 * n[i]
                       * square(
                                m[i]
                                - (a[1]
                                   * Phi(c[1] + b[mn] * pow(x[i], d[mn]))))
                       * invsigma2[1])
                    * trt_ind[i, mn];
        }
      }
  ...
  } else if (nlevels_a > 1 && nlevels_sigma > 1 && nlevels_BMD == 1
             && nlevels_d == 1) {
    ...
    if (data_type == 1) {
      for (i in 1 : N) {
        for (mn in 1 : nlevels) {
          target += (-0.5 * n[i] * log(2 * pi())
                     + 0.5 * n[i] * log(invsigma2[mn])
                     - 0.5 * (n[i] - 1) * s2[i] * invsigma2[mn]
                     - 0.5 * n[i]
                       * square(
                                m[i]
                                - (a[mn]
                                   * Phi(c[mn] + b[mn] * pow(x[i], d[1]))))
                       * invsigma2[mn])
                    * trt_ind[i, mn];
        }
      }
  ...
  } else if (nlevels_d > 1 && nlevels_BMD > 1 && nlevels_a > 1
             && nlevels_sigma > 1) {
  ...
  } else //no dependence on covariates
  if (data_type == 1) {
    for (i in 1 : N) {
      for (mn in 1 : nlevels) {
        target += (-0.5 * n[i] * log(2 * pi())
