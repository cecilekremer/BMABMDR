...
transformed parameters{
  ...
  if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[1]=(-log(1-(q/(c[1]-1)))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
      }else if(data_type == 2){
        b[1]=(-log(1-(log(1+q)/(a[1]*(c[1]-1))))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
      }else if(data_type == 3){
        b[1]=(-log(1-((-q)/(c[1]-1)))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
      }else if(data_type == 4){
        b[1]=(-log(1-(log(1-q)/(a[1]*(c[1]-1))))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=(-log(1-(q/(c[mn]-1)))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
      }else if(data_type == 2){
        b[mn]=(-log(1-(log(1+q)/(a[mn]*(c[mn]-1))))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
      }else if(data_type == 3){
        b[mn]=(-log(1-((-q)/(c[mn]-1)))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
      }else if(data_type == 4){
        b[mn]=(-log(1-(log(1-q)/(a[mn]*(c[mn]-1))))) / (exp(k[1])+(exp(k[1])*(exp(k[1])-1)/exp(d[1])));
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=(-log(1-(q/(c[1]-1)))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
      }else if(data_type == 2){
        b[mn]=(-log(1-(log(1+q)/(a[1]*(c[1]-1))))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
      }else if(data_type == 3){
        b[mn]=(-log(1-((-q)/(c[1]-1)))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
      }else if(data_type == 4){
        b[mn]=(-log(1-(log(1-q)/(a[1]*(c[1]-1))))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
    ...
    for(mn in 1:nlevels_b){
      ...
      if(data_type == 1){
        b[mn]=(-log(1-(q/(c[mn]-1)))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
      }else if(data_type == 2){
        b[mn]=(-log(1-(log(1+q)/(a[mn]*(c[mn]-1))))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
      }else if(data_type == 3){
        b[mn]=(-log(1-((-q)/(c[mn]-1)))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
      }else if(data_type == 4){
        b[mn]=(-log(1-(log(1-q)/(a[mn]*(c[mn]-1))))) / (exp(k[mn])+(exp(k[mn])*(exp(k[mn])-1)/exp(d[mn])));
...
model{
  ...
  for(i in 1:nlevels_sigma){
    par5[i] ~ normal(priormuQ[5,i],priorSigmaQ[5,5]);
  }

  //d prior
  for(i in 1:nlevels_d){
    par4[i] ~ normal(priormuQ[4,i],priorSigmaQ[4,4])T[,truncdQ];
  }
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          ...
          0.5*n[i]*square(m[i]-a[1]-a[1]*(c[1]-1)*(1-exp((-b[mn]*x[i])-((b[mn]/exp(d[mn]))*x[i]*(x[i]-1)))))*
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          ...
          0.5*n[i]*square(m[i]-a[mn]-a[mn]*(c[mn]-1)*(1-exp((-b[mn]*x[i])-((b[mn]/exp(d[1]))*x[i]*(x[i]-1)))))*
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          ...
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[mn])-
          0.5*(n[i]-1)*s2[i]*invsigma2[mn]-
          0.5*n[i]*square(m[i]-a[mn]-a[mn]*(c[mn]-1)*(1-exp((-b[mn]*x[i])-((b[mn]/exp(d[mn]))*x[i]*(x[i]-1)))))*
          invsigma2[mn])*trt_ind[i,mn];
    ...
    if(data_type == 1 || data_type == 3){
      for (i in 1:N){
        for(mn in 1:nlevels){
          ...
          target += (-0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2[1])-
          0.5*(n[i]-1)*s2[i]*invsigma2[1]-
          0.5*n[i]*square(m[i]-a[1]-a[1]*(c[1]-1)*(1-exp((-b[1]*x[i])-((b[1]/exp(d[1]))*x[i]*(x[i]-1)))))*
          invsigma2[1])*trt_ind[i,mn];
