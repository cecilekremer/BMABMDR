...
transformed parameters {
  ...
  if (nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        b[1] = (-log(1 - (q / (c[1] - 1))))
               / (exp(k[1]) + (exp(k[1]) * (exp(k[1]) - 1) / exp(d[1])));
      } else if (data_type == 2) {
        b[1] = (-log(1 - (log(1 + q) / (a[1] * (c[1] - 1)))))
               / (exp(k[1]) + (exp(k[1]) * (exp(k[1]) - 1) / exp(d[1])));
      } else if (data_type == 3) {
        b[1] = (-log(1 - ((-q) / (c[1] - 1))))
               / (exp(k[1]) + (exp(k[1]) * (exp(k[1]) - 1) / exp(d[1])));
    ...
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        ...
        b[mn] = (-log(1 - (log(1 + q) / (a[mn] * (c[mn] - 1)))))
                / (exp(k[1]) + (exp(k[1]) * (exp(k[1]) - 1) / exp(d[1])));
      } else if (data_type == 3) {
        b[mn] = (-log(1 - ((-q) / (c[mn] - 1))))
                / (exp(k[1]) + (exp(k[1]) * (exp(k[1]) - 1) / exp(d[1])));
    ...
    for (mn in 1 : nlevels_b) {
      if (data_type == 1) {
        b[mn] = (-log(1 - (q / (c[1] - 1))))
                / (exp(k[mn]) + (exp(k[mn]) * (exp(k[mn]) - 1) / exp(d[mn])));
      } else if (data_type == 2) {
        b[mn] = (-log(1 - (log(1 + q) / (a[1] * (c[1] - 1)))))
                / (exp(k[mn]) + (exp(k[mn]) * (exp(k[mn]) - 1) / exp(d[mn])));
      } else if (data_type == 3) {
        b[mn] = (-log(1 - ((-q) / (c[1] - 1))))
                / (exp(k[mn]) + (exp(k[mn]) * (exp(k[mn]) - 1) / exp(d[mn])));
