...
//  real m[N];
  ...
  if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1){
    for(mn in 1:nlevels_b){
      expr = q*(1-a[1])+a[1];
      if(expr < 1.0){
        b[mn] = (inv_Phi(q*(1-a[1])+a[1]) - inv_Phi(a[1]))/exp(k[1]*d[1]);
      }else{
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[1]))/exp(k[1]*d[1]);
      }
    }
    ...
    for(mn in 1:nlevels_b){
      expr = q*(1-a[mn])+a[mn];
      if(expr < 1.0){
        b[mn] = (inv_Phi(q*(1-a[mn])+a[mn]) - inv_Phi(a[mn]))/exp(k[1]*d[1]);
      }else{
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[mn]))/exp(k[1]*d[1]);
      }
    }
  }else if(nlevels_a == 1 && nlevels_d > 1 && nlevels_BMD > 1){
    for(mn in 1:nlevels_b){
      expr = q*(1-a[1])+a[1];
      if(expr < 1.0){
        b[mn] = (inv_Phi(q*(1-a[1])+a[1]) - inv_Phi(a[1]))/exp(k[mn]*d[mn]);
      }else{
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[1]))/exp(k[mn]*d[mn]);
      }
    }
  }else if(nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1){
    for(mn in 1:nlevels_b){
      expr = q*(1-a[mn])+a[mn];
      if(expr < 1.0){
        b[mn] = (inv_Phi(q*(1-a[mn])+a[mn]) - inv_Phi(a[mn]))/exp(k[mn]*d[mn]);
      }else{
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[mn]))/exp(k[mn]*d[mn]);
      }
    }
  }
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[1]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((Phi(inv_Phi(a[1]) + b[mn]*pow(x[i], d[mn]))) +eps) +
                                        (n[i] - y[i]) * log(1 - (Phi(inv_Phi(a[1]) + b[mn]*pow(x[i], d[mn]))) +eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a > 1 && nlevels_d == 1 && nlevels_BMD == 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[mn]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((Phi(inv_Phi(a[mn]) + b[mn]*pow(x[i], d[1]))) +eps) +
                                        (n[i] - y[i]) * log(1 - (Phi(inv_Phi(a[mn]) + b[mn]*pow(x[i], d[1])))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[mn]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((Phi(inv_Phi(a[mn]) + b[mn]*pow(x[i], d[mn]))) +eps) +
                                        (n[i] - y[i]) * log(1 - (Phi(inv_Phi(a[mn]) + b[mn]*pow(x[i], d[mn])))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[1]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((Phi(inv_Phi(a[1]) + b[1]*pow(x[i], d[1]))) +eps) +
                                        (n[i] - y[i]) * log(1 - (Phi(inv_Phi(a[1]) + b[1]*pow(x[i], d[1])))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }
