functions {
  real pert_dist_lpdf(real theta, real lb, real md, real ub, real gama) {
    real x1;
    real x2;
    real x3;
    real x4;
    real alpha;
    real beta;
    
    alpha = 1 + gama * (md - lb) / (ub - lb);
    beta = 1 + gama * (ub - md) / (ub - lb);
    
    x1 = (alpha - 1) * log(theta - lb);
    x2 = (beta - 1) * log(ub - theta);
    x3 = (alpha + beta - 1) * log(ub - lb);
    x4 = lbeta(alpha, beta);
    return x1 + x2 - x3 - x4;
  }
}
data {
  int N; // the total number of distinct dose group
  int nlevels;
  int nlevels_a;
  int nlevels_BMD;
  int nlevels_d;
  int nlevels_b;
  vector[N] n; // the sample size for each dose group
  vector[N] x; // the dose level of each dose group
  vector[N] y; // the number of adverse events for each dose group
  matrix[N, nlevels] trt_ind;
  real q; // the BMR
  matrix[4, nlevels] priormu;
  matrix[4, nlevels] priorlb; //lower bound
  matrix[4, nlevels] priorub; //upper bound
  matrix[2, nlevels] priorgama;
  real eps;
  cov_matrix[3] priorSigma;
  real truncd;
}
parameters {
  array[nlevels_a] real<lower=0, upper=1> par1; // 1
  array[nlevels_BMD] real<lower=0> par2; // BMD
  array[nlevels_d] real par3; // d on a log scale
}
transformed parameters {
  array[nlevels_a] real a;
  array[nlevels_b] real b;
  array[nlevels_d] real d;
  array[nlevels_BMD] real k;
  //  real m[N];
  array[nlevels_BMD] real<lower=0> BMD;
  real expr;
  
  for (mn in 1 : nlevels_BMD) {
    BMD[mn] = par2[mn];
    k[mn] = log(par2[mn]);
  }
  
  for (mn in 1 : nlevels_a) {
    a[mn] = par1[mn];
  }
  
  for (mn in 1 : nlevels_d) {
    d[mn] = exp(par3[mn]);
  }
  
  if (nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (mn in 1 : nlevels_b) {
      expr = q * (1 - a[1]) + a[1];
      if (expr < 1.0) {
        b[mn] = (inv_Phi(q * (1 - a[1]) + a[1]) - inv_Phi(a[1]))
                / exp(k[1] * d[1]);
      } else {
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[1])) / exp(k[1] * d[1]);
      }
    }
  } else if (nlevels_a > 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (mn in 1 : nlevels_b) {
      expr = q * (1 - a[mn]) + a[mn];
      if (expr < 1.0) {
        b[mn] = (inv_Phi(q * (1 - a[mn]) + a[mn]) - inv_Phi(a[mn]))
                / exp(k[1] * d[1]);
      } else {
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[mn])) / exp(k[1] * d[1]);
      }
    }
  } else if (nlevels_a == 1 && nlevels_d > 1 && nlevels_BMD > 1) {
    for (mn in 1 : nlevels_b) {
      expr = q * (1 - a[1]) + a[1];
      if (expr < 1.0) {
        b[mn] = (inv_Phi(q * (1 - a[1]) + a[1]) - inv_Phi(a[1]))
                / exp(k[mn] * d[mn]);
      } else {
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[1])) / exp(k[mn] * d[mn]);
      }
    }
  } else if (nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1) {
    for (mn in 1 : nlevels_b) {
      expr = q * (1 - a[mn]) + a[mn];
      if (expr < 1.0) {
        b[mn] = (inv_Phi(q * (1 - a[mn]) + a[mn]) - inv_Phi(a[mn]))
                / exp(k[mn] * d[mn]);
      } else {
        b[mn] = (inv_Phi(0.9999) - inv_Phi(a[mn])) / exp(k[mn] * d[mn]);
      }
    }
  }
}
model {
  // a prior
  for (i in 1 : nlevels_a) {
    par1[i] ~ pert_dist(priorlb[1, i], priormu[1, i], priorub[1, i],
                        priorgama[1, i]);
  }
  
  // BMD prior
  for (i in 1 : nlevels_BMD) {
    par2[i] ~ pert_dist(priorlb[2, i], priormu[2, i], priorub[2, i],
                        priorgama[2, i]);
  }
  
  // d prior
  for (i in 1 : nlevels_d) {
    par3[i] ~ normal(priormu[3, i], priorSigma[3, 3]) T[ , truncd];
  }
  
  if (nlevels_d > 1 && nlevels_BMD > 1 && nlevels_a == 1) {
    for (i in 1 : N) {
      for (mn in 1 : nlevels) {
        if (x[i] == 0) {
          target += (lchoose(n[i], y[i]) + y[i] * log(a[1] + eps)
                     + (n[i] - y[i]) * log(1 - a[1] + eps))
                    * trt_ind[i, mn];
        } else if (x[i] > 0) {
          target += (lchoose(n[i], y[i])
                     + y[i]
                       * log(
                             Phi(inv_Phi(a[1]) + b[mn] * pow(x[i], d[mn]))
                             + eps)
                     + (n[i] - y[i])
                       * log(
                             1
                             - Phi(inv_Phi(a[1]) + b[mn] * pow(x[i], d[mn]))
                             + eps))
                    * trt_ind[i, mn];
        }
      }
    }
  } else if (nlevels_a > 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (i in 1 : N) {
      for (mn in 1 : nlevels) {
        if (x[i] == 0) {
          target += (lchoose(n[i], y[i]) + y[i] * log(a[mn] + eps)
                     + (n[i] - y[i]) * log(1 - a[mn] + eps))
                    * trt_ind[i, mn];
        } else if (x[i] > 0) {
          target += (lchoose(n[i], y[i])
                     + y[i]
                       * log(
                             Phi(inv_Phi(a[mn]) + b[mn] * pow(x[i], d[1]))
                             + eps)
                     + (n[i] - y[i])
                       * log(
                             1
                             - Phi(inv_Phi(a[mn]) + b[mn] * pow(x[i], d[1]))
                             + eps))
                    * trt_ind[i, mn];
        }
      }
    }
  } else if (nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1) {
    for (i in 1 : N) {
      for (mn in 1 : nlevels) {
        if (x[i] == 0) {
          target += (lchoose(n[i], y[i]) + y[i] * log(a[mn] + eps)
                     + (n[i] - y[i]) * log(1 - a[mn] + eps))
                    * trt_ind[i, mn];
        } else if (x[i] > 0) {
          target += (lchoose(n[i], y[i])
                     + y[i]
                       * log(
                             Phi(inv_Phi(a[mn]) + b[mn] * pow(x[i], d[mn]))
                             + eps)
                     + (n[i] - y[i])
                       * log(
                             1
                             - Phi(inv_Phi(a[mn]) + b[mn] * pow(x[i], d[mn]))
                             + eps))
                    * trt_ind[i, mn];
        }
      }
    }
  } else if (nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1) {
    for (i in 1 : N) {
      for (mn in 1 : nlevels) {
        if (x[i] == 0) {
          target += (lchoose(n[i], y[i]) + y[i] * log(a[1] + eps)
                     + (n[i] - y[i]) * log(1 - a[1] + eps))
                    * trt_ind[i, mn];
        } else if (x[i] > 0) {
          target += (lchoose(n[i], y[i])
                     + y[i]
                       * log(
                             Phi(inv_Phi(a[1]) + b[1] * pow(x[i], d[1]))
                             + eps)
                     + (n[i] - y[i])
                       * log(
                             1 - Phi(inv_Phi(a[1]) + b[1] * pow(x[i], d[1]))
                             + eps))
                    * trt_ind[i, mn];
        }
      }
    }
  }
}


