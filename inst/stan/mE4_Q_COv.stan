...
parameters {
  array[nlevels_a] real<lower=0, upper=1> par1; // a
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[1]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[1] + (1-a[1])*(1-exp(-b[mn]*pow(x[i], d[mn])))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[1] + (1-a[1])*(1-exp(-b[mn]*pow(x[i], d[mn]))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a > 1 && nlevels_d == 1 && nlevels_BMD == 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[mn]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[mn] + (1-a[mn])*(1-exp(-b[1]*pow(x[i], d[1])))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[mn] + (1-a[mn])*(1-exp(-b[1]*pow(x[i], d[1]))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[mn]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[mn] + (1-a[mn])*(1-exp(-b[mn]*pow(x[i], d[mn])))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[mn] + (1-a[mn])*(1-exp(-b[mn]*pow(x[i], d[mn]))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[1]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[1] + (1-a[1])*(1-exp(-b[1]*pow(x[i], d[1])))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[1] + (1-a[1])*(1-exp(-b[1]*pow(x[i], d[1]))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }
