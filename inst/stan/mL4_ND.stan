...
parameters{
  vector[5] par; // at=a, BMD on log-scale, ct=log(c), dt=log(d), invsigma2t=log(invsigma2)
}
transformed parameters{
  ...
  a=exp(par[1]);
//  c=par[3];
  c=pow(3,0.5)/pi()*logit(1-q) - exp(par[3]);
  d=exp(par[4]);
  b=exp(-par[2]*d)*(pow(3,0.5)/pi()*logit(inv_logit(pi()/pow(3,0.5)*c)+q)-c);
  invsigma2=exp(par[5]);
}
model{
   ...
   for (i in 1:N){
     target += -0.5*n[i]*log(2*pi())+0.5*n[i]*log(invsigma2)-0.5*(n[i]-1)*s2[i]*invsigma2-0.5*n[i]*square(m[i]-
     ((a*(1+inv_logit(pi()/pow(3,0.5)*c)))-(a*inv_logit(pi()/pow(3,0.5)*(c+b*pow(x[i],d))))))*invsigma2;
...
min_response = a;
max_response = a*inv_logit(pi()/pow(3,0.5)*c);
BMD = exp(par[2]);
