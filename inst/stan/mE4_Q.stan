...
parameters{
  //vector[3] par; // at=logit(a), BMD on log-scale, dt=log(d)
...
transformed parameters{
  ...
  if(is_betabin == 1) {
    ...
    for(i in 1:N){
      abet[i] = m[i]*((1/rho[is_betabin])-1.0);
      bbet[i] = (1.0 - m[i])*((1.0/rho[is_betabin])-1);
    }
  } else if(is_bin == 1) {
    for(i in 1:N){
      abet[i] = 0.0;
...
model{
    ...
    if(is_bin==1) {
      ...
      etarho[is_betabin] ~ normal(priormu[3], priorSigma[3,3]);
      for(i in 1:N){
