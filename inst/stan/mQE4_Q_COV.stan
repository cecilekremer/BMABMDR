...
//  real m[N];
  ...
  if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1){
    for(mn in 1:nlevels_b){
      b[mn] = (-log(1-q)) / (exp(k[1])+(((exp(k[1])*(exp(k[1])-1))/exp(d[1]))));
    }
    ...
    for(mn in 1:nlevels_b){
      b[mn] = (-log(1-q)) / (exp(k[mn])+(((exp(k[mn])*(exp(k[mn])-1))/exp(d[mn]))));
    }
  }else if(nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1){
    for(mn in 1:nlevels_b){
      b[mn] = (-log(1-q)) / (exp(k[mn])+(((exp(k[mn])*(exp(k[mn])-1))/exp(d[mn]))));
...
model{
 ...
 // a prior
  for(i in 1:nlevels_a){
    par1[i] ~ pert_dist(priorlb[1,i], priormuQ[1, i], priorub[1, i], priorgama[1, i]);
  }

  // BMD prior
  for(i in 1:nlevels_BMD){
    par2[i] ~ pert_dist(priorlb[2, i], priormuQ[2, i], priorub[2, i], priorgama[2, i]);
  }

  // d prior
  for(i in 1:nlevels_d){
    par3[i] ~ normal(priormuQ[3, i], priorSigmaQ[3, 3])T[,truncdQ];
  }
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[1]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[1] + (1-a[1])*(1 - exp(-b[mn]*x[i] - ( (b[mn]/exp(d[mn])) * x[i] * (x[i] - 1) )))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[1] + (1-a[1])*(1 - exp(-b[mn]*x[i] - ( (b[mn]/exp(d[mn])) * x[i] * (x[i] - 1) ))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a > 1 && nlevels_d == 1 && nlevels_BMD == 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[mn]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[mn] + (1-a[mn])*(1 - exp(-b[1]*x[i] - ( (b[1]/exp(d[1])) * x[i] * (x[i] - 1) )))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[mn] + (1-a[mn])*(1 - exp(-b[1]*x[i] - ( (b[1]/exp(d[1])) * x[i] * (x[i] - 1) ))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a > 1 && nlevels_d > 1 && nlevels_BMD > 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[mn]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[mn] + (1-a[mn])*(1 - exp(-b[mn]*x[i] - ( (b[mn]/exp(d[mn])) * x[i] * (x[i] - 1) )))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[mn] + (1-a[mn])*(1 - exp(-b[mn]*x[i] - ( (b[mn]/exp(d[mn])) * x[i] * (x[i] - 1) ))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }else if(nlevels_a == 1 && nlevels_d == 1 && nlevels_BMD == 1){
      ...
      for(mn in 1:nlevels){
        if(x[i] == 0) { target += ( lchoose(n[i], y[i]) +
                  ...
                  (n[i] - y[i]) * log(1 - a[1]+eps)) * trt_ind[i, mn];
                                         ...
                                         y[i] * log((a[1] + (1-a[1])*(1 - exp(-b[1]*x[i] - ( (b[1]/exp(d[1])) * x[i] * (x[i] - 1) )))) +eps) +
                                        (n[i] - y[i]) * log(1 - (a[1] + (1-a[1])*(1 - exp(-b[1]*x[i] - ( (b[1]/exp(d[1])) * x[i] * (x[i] - 1) ))))+eps)) * trt_ind[i, mn];
        }
      }
    }
  }
