...
test_that("Prepare data (individual)", {
...
test_that("Prepare data (summary)", {
...
test_that("Laplace approximation", {
      ...
      # plot output
      pFLBMD = plot.BMADR(FLBMD, weight_type = "LP", include_data = T, all = F, title = '', log = T)
...
test_that("Sampling", {
      ...
      # plot output
      pSBMD = plot.BMADR(SBMD, weight_type = "BS", include_data = T, all = F, title = '', log = T)
      pSBMD$BMDs
      pSBMD$weights
      pSBMD$model_fit_N
      pSBMD$model_fit_LN
